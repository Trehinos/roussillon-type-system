...
impl Gadt {
    pub fn new(constructors: &[FunctionDefinition]) -> Self {
        Gadt { constructors: constructors.to_vec() }
    }

    pub fn to_sequence_type(&self) -> SequenceType {
        let mut sequence_type = SequenceType::new();
        for constructor in &self.constructors {
            let constructor_type = constructor.signature.clone();
    ...
    pub fn apply(&self, tag: usize, arguments: &Sequence) -> Option<ValueCell> {
        ...
        let expected_type = constructor.signature.arguments.typename();
